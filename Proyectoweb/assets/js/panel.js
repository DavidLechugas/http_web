...
(function() {
  ...
  function cargarSesionUsuario() {
    ...
    if (initialEl) initialEl.textContent = usuario.avatarLetter || usuario.nombre.charAt(0).toUpperCase();
  }
  ...
  function renderizarTabla() {
    ...
    let lista = piezas.filter(item => {
      ...
      if (busquedaActual.trim() !== "") {
        ...
        const texto = (item.nombre + " " + item.origen + " " + item.categoria + " " + (item.desc || "")).toLowerCase();
    ...
    if (lista.length === 0) {
      tbody.innerHTML = '<tr><td colspan="4" class="text-center py-4 text-muted"><i class="bi bi-inbox fs-3 d-block mb-1"></i>No se encontraron piezas con los criterios seleccionados.</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = lista.map(item => {
      return '<tr>' +
        '<td>' +
          ...
          '<small class="text-muted">' + item.categoria + ' &bull; $' + item.precio + ' COP</small>' +
        '</td>' +
        '<td>' +
          '<span class="text-secondary"><i class="bi bi-geo-alt text-success me-1"></i>' + item.origen + '</span>' +
        '</td>' +
        '<td>' +
          '<span class="badge ' + item.badge + '">' + item.estado + '</span>' +
        '</td>' +
        '<td class="text-end">' +
          '<div class="btn-group btn-group-sm">' +
            '<button class="btn btn-outline-success btn-action-pill btn-ver-detalle-pieza" data-id="' + item.id + '" title="Ver detalles y avance">' +
              '<i class="bi bi-eye me-1"></i> Detalles' +
            '</button>' +
            '<button class="btn btn-outline-danger btn-action-pill btn-eliminar-pieza" data-id="' + item.id + '" title="Eliminar pieza">' +
  ...
  function abrirModalDetalle(id) {
    ...
    if (badgeEl) {
      ...
      badgeEl.className = "badge " + pieza.badge;
    }
    if (descEl) descEl.textContent = pieza.desc;
    if (catEl) catEl.textContent = pieza.categoria;
    if (precioEl) precioEl.textContent = "$" + pieza.precio + " COP";
    ...
    if (progBar) {
      ...
      progBar.className = "progress-bar " + (pieza.estado === 'En exhibición' ? 'bg-success' : 'bg-warning');
    }
  ...
  function eliminarPieza(id) {
    ...
    if (confirm("¿Estás seguro de que deseas retirar \"" + pieza.nombre + "\" del catálogo de producción?")) {
  ...
  function actualizarEstadoPieza() {
    ...
    alert("¡Estado de \"" + pieza.nombre + "\" actualizado a \"" + nuevoEstado + "\" con éxito!");
  }
  ...
  function prepararReporte() {
    ...
    if (reportDate) reportDate.textContent = "Generado: " + hoy;
    ...
    if (tbody) {
      tbody.innerHTML = piezas.map(p => {
        return '<tr>' +
          ...
          '<td>' + p.origen + '</td>' +
          '<td>' + p.categoria + '</td>' +
          '<td><span class="badge ' + p.badge + '">' + p.estado + '</span></td>' +
        '</tr>';
      }).join('');
    }
  }
  ...
  function descargarReporteCSV() {
    ...
    a.href = url;
    a.download = "Reporte_Produccion_Artesanias_" + new Date().toISOString().slice(0, 10) + ".csv";
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    if (formNueva) {
      formNueva.addEventListener('submit', function(e) {
        ...
        formNueva.reset();
        alert("¡\"" + nombre + "\" ha sido registrada exitosamente en el catálogo de producción!");
      });
    }
